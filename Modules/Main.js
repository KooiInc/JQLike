...
async function importStuff(link) {
  const cdnLink = {
    link,
    get version() { return this.link.split("@")[1].split("/")[0] || "master"; }
  };

  import(cdnLink.link).then(r => run(r.$, r.util));
  ...
  function run($, util) {
      ...
      $([
        `<h2>Testing a JQ-alike html helper library (version: ${cdnLink.version})</h2>`,
        `<p data-starttext>
